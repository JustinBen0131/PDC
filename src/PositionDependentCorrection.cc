...
int PositionDependentCorrection::End(PHCompositeNode* /*topNode*/)
{
  ...
  if (Verbosity() > 0)
  {
    ...
    if (m_etaWinCLUScp)
    {
      ...
      /* ────────────────────────────────────────────────────────────────── *
       *  Extra diagnostics – printed only at very high verbosity (> 6)     *
       *    • lists the N busiest (ηloc , φloc) bins in the *uncorrected*   *
       *      block‑coordinate histogram                                    *
       *    • gives quick “strip” shares for ηloc ≈ 0/1 and φloc ≈ 0/1      *
       * ────────────────────────────────────────────────────────────────── */
      if (Verbosity() > 6 && h3_cluster_block_cord_E)
      {
          std::cout << '\n' << ANSI_BOLD
                    << "╭───────────────────────────────────────────────╮\n"
                    << "│    Top‑occupancy bins in h3_cluster_block     │\n"
                    << "╰───────────────────────────────────────────────╯"
                    << ANSI_RESET << '\n';

          /* 1) project (ηloc , φloc , E) → (ηloc , φloc) */
          TH2D* h2_local =
              static_cast<TH2D*>( h3_cluster_block_cord_E->Project3D("yx") );

          struct BinInfo { double cnt; int ix; int iy; };
          std::vector<BinInfo> v;
          v.reserve( h2_local->GetNbinsX() * h2_local->GetNbinsY() );

          for (int ix = 1; ix <= h2_local->GetNbinsX(); ++ix)
            for (int iy = 1; iy <= h2_local->GetNbinsY(); ++iy)
            {
                const double c = h2_local->GetBinContent(ix, iy);
                if (c > 0.0) v.push_back({c, ix, iy});
            }

          std::sort(v.begin(), v.end(),
                    [](const BinInfo& a, const BinInfo& b){ return a.cnt > b.cnt; });

          const int Nshow = std::min<int>(8, v.size());

          std::cout << std::left << std::setw(6)  << "rank"
                    << std::setw(12) << "ηloc"
                    << std::setw(12) << "φloc"
                    << std::right<< std::setw(14)<< "entries\n"
                    << "───────────────────────────────────────────────\n";

          for (int k = 0; k < Nshow; ++k)
          {
              const auto& b = v[k];
              const double etaLoc = h2_local->GetXaxis()->GetBinCenter(b.ix);
              const double phiLoc = h2_local->GetYaxis()->GetBinCenter(b.iy);

              std::cout << std::left << std::setw(6)  << k+1
                        << std::setw(12) << Form("%+.3f", etaLoc)
                        << std::setw(12) << Form("%+.3f", phiLoc)
                        << std::right<< std::setw(14)<< static_cast<std::uint64_t>(b.cnt)
                        << '\n';
          }
          ...
          const double tot = h2_local->GetEntries();
          const int    iEta0 = h_eta_proj->FindBin(0.0);
          const int    iEta1 = h_eta_proj->FindBin(1.0);
          const int    iPhi0 = h_phi_proj->FindBin(0.0);
          const int    iPhi1 = h_phi_proj->FindBin(1.0);

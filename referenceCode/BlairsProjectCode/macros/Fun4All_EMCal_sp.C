...
void Fun4All_EMCal_sp(int nevents = 5e2, const std::string &fname = "condor/listFiles/gamma_dst_calo_cluster.list",const std::string &fnamehits = "condor/listFiles/gamma_g4hits.list")
{
 ...
 // pi0EtaByEta *ca2 = new pi0EtaByEta("calomodulename","pi0_"+ OutFile);
 // ca2->set_reqMinBias(false);
 // ca2->set_RunTowByTow(false); // to decide if we want to run tbt (default is true)
 // ca2->set_RunTBTCompactMode(true);
 // ca2->apply_vertex_cut(true);
 // ca2->set_pt1BaseClusCut(1.0);
 // ca2->set_pt2BaseClusCut(0.6);
 // ca2->set_NclusDeptFac(0.0);
 // se->registerSubsystem(ca2);
